...
export function DateTimePicker({
	...
	function handleDateSelect(date: Date | undefined) {
		if (date && !disabled) {
			...
			if (date.getTime() === today.getTime()) {
				...
				nowPlusOneHour.setHours(nowPlusOneHour.getHours() + 1); // Add 1 hour to the current time
				newDateTime.setHours(nowPlusOneHour.getHours());
				newDateTime.setMinutes(nowPlusOneHour.getMinutes());
			} else {
	...
	function handleTimeChange(type: "hour" | "minute" | "ampm", value: string) {
		...
		const isToday = baseDateOnly.getTime() === today.getTime();
	...
	function validateTimeLogic(newDateTime: Date) {
		// Make sure arrival time is after departure time and vice versa
		if (form_value === "departureTime" && form.getValues("arrivalTime")) {
			const arrivalTime = new Date(form.getValues("arrivalTime"));
			if (newDateTime >= arrivalTime) {
				setTimeValidMessage?.(t("timeValidattion.departureBeforeArrival"));
				return;
			}
		} else if (form_value === "arrivalTime" && form.getValues("departureTime")) {
			const departureTime = new Date(form.getValues("departureTime"));
			if (newDateTime <= departureTime) {
				setTimeValidMessage?.(t("timeValidattion.arrivalAfterDeparture"));
				return;
			}
		}

		if (form.getValues("type") === TripType.RoundTrip) {
			if (form_value === "returnDepartureTime" && form.getValues("returnArrivalTime")) {
				const returnArrivalTime = new Date(form.getValues("returnArrivalTime"));
				if (newDateTime >= returnArrivalTime) {
					setTimeValidMessage?.(t("timeValidattion.returnDepartureBeforeReturnArrival"));
					return;
				}
			} else if (form_value === "returnArrivalTime" && form.getValues("returnDepartureTime")) {
				const returnDepartureTime = new Date(form.getValues("returnDepartureTime"));
				if (newDateTime <= returnDepartureTime) {
					setTimeValidMessage?.(t("timeValidattion.returnArrivalAfterReturnDeparture"));
					return;
				}
			}
		}
