...
export const StopsColumn = ({ stops: stopData }: StopsColumnProps) => {
	const [stops] = useState(stopData);
	...
	const handleRemoveStop = () => {
		toast.promise(new Promise((r) => setTimeout(r, 3000)), {
			...
			error: (e) => {
				...
				return `Could not remove request #PlaceholderId, please try again later`;
			},
		});
	};
					...
					<TimelineHeader>
						<TimelineDate>
							{stop.actualArrivalTime
								? `Arrived: ${format(stop.actualArrivalTime, dateTimeFormat)}`
								: `Est. ${format(stop.arrivalTime, dateTimeFormat)}`}
						</TimelineDate>
						<TimelineTitle>
							...
							<AlertDialogContent>
								<AlertDialogHeader>
									<AlertDialogTitle>
										...
										request from the trip?
									</AlertDialogTitle>
									<AlertDialogDescription>
										This trip upon removed will return to
										the scheduling pool.
									</AlertDialogDescription>
								</AlertDialogHeader>
								<AlertDialogFooter>
